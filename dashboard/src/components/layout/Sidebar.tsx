...
import { NAV_SECTIONS, sidebarItemsForRole } from '@/lib/constants'
...
export function Sidebar({ onNavigate, mobile = false }: SidebarProps) {
  ...
  const navItems = sidebarItemsForRole(role)
    ...
    <TooltipProvider delayDuration={0}>
      ...
      >
        ...
        <div className="flex h-14 items-center gap-2.5 border-b border-sidebar-border/70 px-3">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-primary/60 bg-primary text-primary-foreground shadow-[0_5px_16px_rgba(37,99,235,0.2)]">
            <BrandMark className="h-[18px] w-[18px]" />
          </div>
          {!isCollapsed && (
            <div className="min-w-0 leading-none">
              <span className="block font-display text-[17px] font-bold tracking-[-0.025em]">ModelPort</span>
              <span className="mt-1 block text-[9px] font-semibold uppercase tracking-[0.18em] text-sidebar-foreground/45">AI Gateway</span>
            </div>
          )}
          {mobile && (
        ...
        <ScrollArea className="flex-1 py-3">
          <nav className="flex flex-col gap-3 px-2">
            {NAV_SECTIONS.filter((section) => section !== '帮助').map((section) => {
